...
class Comments_model extends CI_Model{
   ...
   public function makeComment($media,$email,$content){
     $data = [':media_id' => $media,':email' => $email,':content' => $content,':type' => "comments",':date' => time()];
     $sql = "INSERT INTO tbl_comments (media_id, email, content,type,date) VALUES (:media_id, :email, :content, :type, :date)";
   ...
   public function loadcomments($media=0,$id=0){
       $this->db->select('tbl_comments.*, tbl_android_users.name');
       $this->db->from('tbl_comments');
       $this->db->join('tbl_android_users','tbl_android_users.email=tbl_comments.email');
       $this->db->where('tbl_comments.media_id',$media);
       if($id!=0){
         $this->db->where('tbl_comments.id <',$id);
       }
       ...
       foreach ($result as $res) {
         ...
         //$res->avatar = $this->getUserAvatar($res->email);
   ...
   public function checkIfMediaHasMoreComments($media=0,$id=0){
